...
| File                                                         | What it shows                                                                                                            |
| ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------ |
| [`ai-sdk-catalog.example.json`](ai-sdk-catalog.example.json) | The full placeholder catalog: direct providers (bare and with a vendor block), one gateway provider with three backends, per-model `capabilities`, and roles in both spellings. Price sheets (`cost`) are not written: the catalog fills them from its embedded models.dev snapshot; write `cost` only to pin or correct a price. |
